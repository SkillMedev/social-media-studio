...
Produce a reply sheet: one row per inbound item containing the original comment (or a short excerpt), its lane, the drafted reply ready to paste into the platform — or "no reply" with a one-line reason — and a review flag on anything escalated, so a human can approve the flagged items and post the rest as-is.
